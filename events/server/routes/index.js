...
const privateMiddleware = async (req, res, next) => {
    ...
    if (req.query.key === process.env.KEY && req.query.secret === process.env.SECRET) {
        ...
        if (req.query.token && req.query.token !== "undefined") {
            console.log('get');
            await fetch("http://localhost:8007/session/token/" + req.query.token + "/service/Events", {
                method: "get",
            ...
            console.log('create');
            await fetch("http://localhost:8007/session/token/Events", {
                method: "post",
                headers: {'Content-Type': 'application/json'}
            }).then(response => {
...
router.get('/all/user/:id', privateMiddleware, controllers.getAllEvents);
router.post('/create', privateMiddleware, controllers.createEvent);
router.post('/delete/event/:id', privateMiddleware, controllers.deleteEvent);
